...
#sys.path.append(BASE_DIR.joinpath("data","input","transformers"))
...
def get_logger(filename=OUTPUT_DIR.joinpath('train')):
...
train = pd.read_csv(BASE_PATH_INPUT.joinpath('train.csv'))
train['annotation'] = train['annotation'].apply(ast.literal_eval)
train['location'] = train['location'].apply(ast.literal_eval)
features = pd.read_csv(BASE_PATH_INPUT.joinpath('features.csv'))

def preprocess_features(features):
    features.loc[27, 'feature_text'] = "Last-Pap-smear-1-year-ago"
    return features
features = preprocess_features(features)
patient_notes = pd.read_csv(BASE_PATH_INPUT.joinpath('patient_notes.csv'))
...
def train_loop(folds, fold):
    ...
    torch.save(model.config, OUTPUT_DIR.joinpath('config.pth'))
    model.to(device)
